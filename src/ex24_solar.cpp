...
int main () {
  ...
  //CriaElipse(70, 50 , p1); //Orbita da Terra
  ...
  focoSol = sqrt(pow(70, 2) - pow(50, 2)); //a^2 = b^2 + c^2

  orbitalua = CriaGrupo();
  //CriaElipse(15, 10, p1); //Orbita da Lua
  for(ang = 0; ang < 2*PI; ang+= 0.01){
    p1.x = 13*cos(ang);
    p1.y = 11*sin(ang);
  ...
  focoTerra = sqrt(pow(13, 2) - pow(11, 2)); //a^2 = b^2 + c^2
  ...
  for (ang = 0;  ; ang +=.01) {
    p2.x = 70*cos(ang);
    p2.y = 50*sin(ang);
    Move(p2,terra);//translada Terra para sua orbita

    p3.x = 13*cos(ang*15)+ p2.x + focoTerra;
    p3.y = 11*sin(ang*15)+ p2.y;
    Move(p3,lua);//translada Lua para sua orbita
  ...
  Desenha ();//(desnecessario) mantem janela aberta
  return 0;
}
